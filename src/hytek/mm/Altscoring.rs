...
impl Altscoring {
    ...
    fn from_csv_path(csv: String) -> std::result::Result<Vec<Altscoring>, Box<dyn Error>> {
       let mut vec: Vec<Altscoring> = Vec::new();
       let mut rdr = ReaderBuilder::new().has_headers(false).from_path(csv)?;
       for elem in rdr.deserialize() {
            let obj: Altscoring = result?;
            vec.push(obj);
       }
       Ok(vec)
    }
    fn from_mdb(conn: Connection) -> Result<Vec<Altscoring>, Error> {
        let mut vec: Vec<Altscoring> = Vec::new();
        match conn.execute("SELECT * FROM alt scoring", ())? {
            Some(mut cursor) => {
