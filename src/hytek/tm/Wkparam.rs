...
impl Wkparam {
    ...
    fn from_csv_path(csv: String) -> std::result::Result<Vec<Wkparam>, Box<dyn Error>> {
       let mut vec: Vec<Wkparam> = Vec::new();
       let mut rdr = ReaderBuilder::new().has_headers(false).from_path(csv)?;
       for elem in rdr.deserialize() {
            let obj: Wkparam = elem?;
            vec.push(obj);
       }
       Ok(vec)
    }
    fn from_mdb(conn: Connection) -> Result<Vec<Wkparam>, Box<dyn Error>> {
        let mut vec: Vec<Wkparam> = Vec::new();
        match conn.execute("SELECT * FROM wk param", ())? {
            Some(mut cursor) => {
