...
impl Workout {
    ...
    fn from_csv_path(csv: String) -> std::result::Result<Vec<Workout>, Box<dyn Error>> {
       let mut vec: Vec<Workout> = Vec::new();
       let mut rdr = ReaderBuilder::new().has_headers(false).from_path(csv)?;
       for elem in rdr.deserialize() {
            let obj: Workout = elem?;
            vec.push(obj);
       }
       Ok(vec)
    }
    fn from_mdb(conn: Connection) -> Result<Vec<Workout>, Box<dyn Error>> {
        let mut vec: Vec<Workout> = Vec::new();
        match conn.execute("SELECT * FROM workout", ())? {
            Some(mut cursor) => {
