...
impl 2016fssl {
    ...
    fn from_csv_path(csv: String) -> std::result::Result<Vec<2016fssl>, Box<dyn Error>> {
       let mut vec: Vec<2016fssl> = Vec::new();
       let mut rdr = ReaderBuilder::new().has_headers(false).from_path(csv)?;
       for elem in rdr.deserialize() {
            let obj: 2016fssl = elem?;
            vec.push(obj);
       }
       Ok(vec)
    }
    fn from_mdb(conn: Connection) -> Result<Vec<2016fssl>, Box<dyn Error>> {
        let mut vec: Vec<2016fssl> = Vec::new();
        match conn.execute("SELECT * FROM 2016FSSL", ())? {
            Some(mut cursor) => {
